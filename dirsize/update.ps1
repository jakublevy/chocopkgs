...
function global:au_GetLatest {
    $rel = (Get-GitHubLatestReleaseLinks -user 'shenwei356' -repository 'dirsize').Links | % href
    $relative_url  = $rel | Where-Object { $_ -match '/shenwei356/dirsize/releases/download/v\d+\.\d+(\.\d+)*/dirsize_windows_amd64\.exe\.tar\.gz' } | Select-Object -First 1
