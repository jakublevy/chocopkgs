...
if($keys.Count -eq 3) { #'Clip Studio Paint' is installed, keeping 'Clip Studio'
  $keys | ? { $_.DisplayName -match 'Clip Studio Modeler*' } | % { $keysToDelete.Add($_) }
}
elseif($keys.Count -le 2 -and $keys.Count -ge 1) { #Removing 'Clip Studio Modeler' and 'Clip Studio'. ('Clip Studio Paint' is not installed)
...
foreach($key in $keysToDelete) {
  ...
  if($key.DisplayName -match 'Modeler') {
    $packageArgs['silentArgs'] += "`"$toolsDir\uninstallModeler.iss`""
  }
  else {
...
if($keys.Count -eq 3) {
  Remove-Item `
    -Path "$quickLaunchFolder\CLIP STUDIO MODELER.lnk" `
    -Force `
...
elseif($keys.Count -le 2) {
  ...
  $desktopIcons = @('CLIP STUDIO.lnk', 'CLIP STUDIO MODELER.lnk')
