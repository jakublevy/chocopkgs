...
function global:au_GetLatest {
    $download_page = Invoke-WebRequest -UseBasicParsing -Uri 'https://sysprogs.com/analyzer2go/download'
    $absoluteUrl   = $download_page.Links | Where-Object href -match '-trial\.msi' | Select-Object -First 1 -expand href
    $version       = ([regex]::Match($absoluteUrl, '/Analyzer2Go-(\d+\.\d+(\.\d+)*(r\d+)?)-trial\.msi')).Groups[1].Value
