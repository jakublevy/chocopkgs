...
function global:au_GetLatest {
    ...
    $relative_url  = $rel | Where-Object { $_ -match '/strawberrymusicplayer/strawberry/releases/download/\d+\.\d+(\.\d+)*/strawberry-.*\.tar.xz' } | Select-Object -First 1
    $version = ([regex]::Match($relative_url, '/(\d+\.\d+(\.\d+)*)/')).Groups[1].Value
    @{
        Version      = $version
        Url32        = "https://codeberg.org/WetOtter44/StrawberryMusicPlayer/releases/download/$version/StrawberrySetup-$version-msvc-x86.exe"
        Url64        = "https://codeberg.org/WetOtter44/StrawberryMusicPlayer/releases/download/$version/StrawberrySetup-$version-msvc-x64.exe"
