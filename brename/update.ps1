...
function global:au_GetLatest {
    $rel = (Get-GitHubLatestReleaseLinks -user 'shenwei356' -repository 'brename').Links | % href
    $relative_url  = $rel | Where-Object { $_ -match '/shenwei356/brename/releases/download/v\d+\.\d+(\.\d+)*/brename_windows_amd64\.exe\.tar\.gz' } | Select-Object -First 1
